...
(function ($) {
  ...
  const common = {
    ...
    calendar: function () {
      ...
      /* made by pjw */
      $('.btn-currentDayNew').on('click', function(){
        $(this).parent().siblings('.date-single-range').find('input').val(start.format('YYYY-MM-DD'));
        dateRange(inValiDate);
      });
      $('.btn-prevWeekNew').on('click', function(){
        var start = moment().subtract(6, 'days');
        $(this).parent().siblings('.date-single-range').find('input').eq(0).val(start.format('YYYY-MM-DD'));
        $(this).parent().siblings('.date-single-range').find('input').eq(1).val(end.format('YYYY-MM-DD'));
        dateRange(inValiDate);
      });
      $('.btn-prevMonthNew').on('click', function(){
      ...
      $('.btn-prevYearNew').on('click', function(){
        var start = moment().subtract(364, 'days');
        $(this).parent().siblings('.date-single-range').find('input').eq(0).val(start.format('YYYY-MM-DD'));
        $(this).parent().siblings('.date-single-range').find('input').eq(1).val(end.format('YYYY-MM-DD'));
        dateRange(inValiDate);
      });
      $('.btn-ResetNew').on('click', function(){
        $('.date-single-range input').val('');
        $('.range input').val('');
      });
      /* made by pjw */
      ...
      function monitoringDate(inValiDate) {
        ...
        $('.monitoring-date #datepicker')
          ...
          .on('cancel.daterangepicker', function(ev, picker) {
            $(this).val(moment().format('YYYY.MM.DD')).trigger("change");
      ...
      function monitoringWeek(inValiDate) {
        ...
        $('.monitoring-date #datepicker')
          ...
          .on('cancel.daterangepicker', function(ev, picker){
            var stDt = moment().isoWeekday(1).format('YYYY.MM.DD');
            var edDt = moment().isoWeekday(7).format('YYYY.MM.DD');
            picker.startDate.isoWeekday(1);
            picker.endDate.isoWeekday(7);
            picker.setStartDate(stDt);
            picker.setEndDate(edDt);
            $(this).val(picker.startDate.format('YYYY.MM.DD') + ' ~ ' + picker.endDate.format('YYYY.MM.DD'));
            $('#date-hidden').val(picker.startDate.clone().subtract(1, 'week').format('YYYY.MM.DD') + ' ~ ' + picker.endDate.clone().subtract(1, 'week').format('YYYY.MM.DD'));
          })
          .on('show.daterangepicker', function(ev, picker){
            $(".daterangepicker .drp-buttons .btn.cancelBtn").attr("style", "display:inline");
            $(".daterangepicker .drp-buttons .btn.applyBtn").attr("style", "display:none");
      ...
      /* 2024-08-26 */
      analysisDate(inValiDate);
      
      $('.analysisDate input').val(start.format('YYYY.MM.DD'));      

      function analysisDate(inValiDate) {
        $('.prev-date').attr('disabled', false);
        $('.analysis-date #dateStart')
          .daterangepicker({
            startDate: start,
            endDate: end,
            minYear: 2022,
      ...
      function analysisMonth(monthInVali) {
        var disabledMonths = monthInVali;

        $('.analysis-date #monthStart')
          .monthpicker({
            pattern: 'yyyy.mm',
            selectedYear: 2024 //
          })         

        $('.analysis-date #monthEnd')
          .monthpicker({
            pattern: 'yyyy.mm',
            selectedYear: 2023 //
          })
      ...
      function analysisCalendar() {
        const $monitorDate = $('.analysis-date');
        $('.prev-date').attr('disabled', false);
        ...
        // 일단위
        $('.btn-todayNew').on('click', function () {
          $('.btn-todayNew, .btn-weekNew, .btn-monthNew').removeClass('active');
          $(this).addClass('active');
          $('.daterangepicker').addClass('monitoring-day');
          $('.analysis-date').removeClass('week month').addClass('day');

          var dateStart = $('#dateStart').data('daterangepicker');
          var dateEnd = $('#dateEnd').data('daterangepicker');
          $('#dateStart').val(end.format('YYYY.MM.DD'));
          $('#dateEnd').val(end.format('YYYY.MM.DD'));          
          $('#date-hidden').val(dateStart.startDate.clone().subtract(1, 'days').format('YYYY.MM.DD'));
          $('#date-hidden').val(dateEnd.startDate.clone().subtract(1, 'days').format('YYYY.MM.DD'));         

          analysisDate(inValiDate);
        });

        // 주단위
        $('.btn-weekNew').on('click', function () {
          $('.btn-todayNew, .btn-weekNew, .btn-monthNew').removeClass('active');
          $(this).addClass('active');

          $('.daterangepicker').addClass('monitoring-week');

          $('.analysis-date').removeClass('day month').addClass('week');

          analysisDate(inValiDate);

          var dateStart = $('#dateStart').data('daterangepicker');
          var dateEnd = $('#dateEnd').data('daterangepicker');
          dateStart.startDate.isoWeekday(1);
          dateStart.endDate.isoWeekday(7);

          dateEnd.startDate.isoWeekday(1);
          dateEnd.endDate.isoWeekday(7);

          $('#dateStart').val(dateStart.startDate.format('YYYY.MM.DD'));
          $('#dateEnd').val(dateEnd.startDate.format('YYYY.MM.DD'));

          $('#date-hidden').val(dateStart.startDate.clone().subtract(1, 'weeks').format('YYYY.MM.DD'));
          $('#date-hidden').val(dateEnd.startDate.clone().subtract(1, 'weeks').format('YYYY.MM.DD'));          
        });

        // 월단위
        $('.btn-monthNew').on('click', function () {
          $('.btn-todayNew, .btn-weekNew, .btn-monthNew').removeClass('active');
          $(this).addClass('active');

          $('.analysis-date').removeClass('day week').addClass('month');

          $('#monthStart').monthpicker('destroy');
          $('#monthEnd').monthpicker('destroy');

          $('#monthStart').val(end.format('YYYY.MM'));
          $('#monthEnd').val(end.format('YYYY.MM'));
          $('#date-hidden').val(end.clone().subtract(1, 'months').format('YYYY.MM'));
...
function tooltips() {
  $('.tooltips').each(function () {
    ...
    $('body').on('click', function (event) {
      ...
      if (!clickedElement.hasClass('btn-search')) {        
        $box.hide();
      }
    });
  });
...
function checkRdo(){
  $(".checkRdo").find(".radio").click(function(){
    var rdoIndex = $(this).index();
    $(".checkContent").eq(rdoIndex).css("display", "table-row").siblings(".checkContent").css("display", "none");
  })
}

//전체 선택/해제
function checkAll(){
  $(".chk-wrap").find("input[type='checkbox']:first").click(function(){
    var others = $(this).parent().siblings().find("input[type='checkbox']");
    if($(this).is(":checked")){
  ...
  $(".chk-wrap").find("input[type='checkbox']:not(:first)").change(function(){
    var $parents = $(this).parents(".chk-wrap");
    var $chk = $parents.find("input[type='checkbox']:not(:first)").length;
    var $checked = $parents.find("input[type='checkbox']:not(:first):checked").length;
    if($chk === $checked){
      $parents.find("input[type='checkbox']:first").prop("checked", true);
    }else{
      $parents.find("input[type='checkbox']:first").prop("checked", false);
    }
  })
}